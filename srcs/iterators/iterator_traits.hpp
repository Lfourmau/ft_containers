...
namespace ft
{
	...
	class normal_iterator
	{
		...
		public:
			...
			typedef typename ft::Iterator_traits<Iter>::reference reference;
			normal_iterator() {};
			normal_iterator (iterator_type it) : iter(it) {};
			template <class Iterator>
  			normal_iterator (const normal_iterator<Iterator>& rev_it) : iter(rev_it.iter) {};
			~normal_iterator() {};

			template< class U >
			normal_iterator& operator=( const normal_iterator<U>& other )
			{ 
				this->iter = other;
				return (*this);
			};
